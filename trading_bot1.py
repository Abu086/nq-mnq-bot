...
def simulate_trade(inst_name: str, inst: dict, spot: float) -> dict:
    ...
    target_profit    = round(max_profit_total * TARGET_PCT, 2)   # 30% of max profit
    stop_loss_amt    = round(entry_premium * SL_PCT * n, 2)      # 30% of premium paid

    # Target and stop loss exit premiums — strictly enforced
    target_exit_premium = round(entry_premium + (max_value - entry_premium) * TARGET_PCT, 2)
...
def _build_summary_sheet(ws):
    ...
    ws["B4"]  = f'=COUNTIF(NQ_Trades!X3:X1000,"PROFIT")'
    ws["B5"]  = f'=COUNTIF(NQ_Trades!X3:X1000,"STOP LOSS")'
    ws["B6"]  = f'=COUNTIF(NQ_Trades!X3:X1000,"FLAT")'
    ...
    ws["B16"] = f'=COUNTIF(MNQ_Trades!X3:X1000,"PROFIT")'
    ws["B17"] = f'=COUNTIF(MNQ_Trades!X3:X1000,"STOP LOSS")'
    ws["B18"] = f'=COUNTIF(MNQ_Trades!X3:X1000,"FLAT")'
