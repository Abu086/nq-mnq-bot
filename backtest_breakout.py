...
LOOKBACK         = 10          # prior candles defining the breakout range
BUDGET           = 2_000       # Rs.2,000 own capital per trade (MIS intraday ~5x margin = Rs.10,000 effective)
SL_PCT           = 0.005       # 0.5%
...
def backtest_stock(symbol: str, df: pd.DataFrame,
    ...
    for trade_date, day_df in df.groupby("date"):
        ...
        # Compute rolling high/low using ONLY THIS DAY's candles
        # shift(1) ensures we use prior candles only (not current candle)
...
def main():
    ...
    # ── Apply daily capital limit ─────────────────────────────────────────
    # With ₹84,000 total budget and ₹2,000 per trade → max 42 trades per day
    # First signals of each day get priority; later ones are skipped if budget exhausted
    DAILY_BUDGET   = 84_000
    CAPITAL_PER_TRADE = BUDGET  # Rs.2,000
    MAX_TRADES_PER_DAY = int(DAILY_BUDGET // CAPITAL_PER_TRADE)

    log.info(f"\nApplying daily capital limit: Rs.{DAILY_BUDGET:,} / Rs.{CAPITAL_PER_TRADE:,} per trade = max {MAX_TRADES_PER_DAY} trades/day")

    # Sort by date and entry time — first signals get priority
    all_trades_df = pd.DataFrame(all_trades).sort_values(["date", "entry_time"])
    filtered = []
    ...
    log.info(f"Trades before capital limit: {len(all_trades)}")
    log.info(f"Trades after capital limit : {len(filtered)}")

    print("\n\n" + "=" * 115)
    print(f"  AFTER APPLYING DAILY BUDGET LIMIT (Rs.{DAILY_BUDGET:,} / Rs.{CAPITAL_PER_TRADE:,} per trade = max {MAX_TRADES_PER_DAY}/day)")
    print("=" * 115)
