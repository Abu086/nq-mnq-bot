...
# ── Strategy Parameters ────────────────────────────────────────────────────
BACKTEST_DAYS    = 60
CANDIDATE_CSV    = "ind_nifty100list.csv"
...
def get_daily_top_n(ranking_df: pd.DataFrame, n: int) -> dict:
    """Returns {date: [top-N symbols by traded value]}."""
    result = {}
    for date, day_df in ranking_df.groupby("date"):
        result[date] = day_df.nlargest(n, "traded_value")["symbol"].tolist()
    return result
...
def backtest_stock(symbol: str, df: pd.DataFrame,
    ...
    # Pre-compute rolling high/low of PRIOR 10 candles (shift(1) avoids look-ahead)
...
def main():
    ...
    # Compute daily top-N (here N = all 100 since pool = 100)
    # We still rank so we can easily change N later
