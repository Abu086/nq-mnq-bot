...
def get_universe(client, token_map):
    """Load today's universe from cache, or fetch and cache it."""
    ...
    for sym, token in token_map.items():
        ...
        if candles:
            c = candles[-1]
            rows.append({"symbol": sym, "traded_value": float(c[4]) * float(c[5])})
        time.sleep(1.0)

    if not rows:
        log.error("Could not rank stocks — no daily candle data")
        return []

    top = pd.DataFrame(rows).nlargest(TOP_N, "traded_value")["symbol"].tolist()
    with open(RANKING_CACHE, "w") as f:
        json.dump({"date": str(today), "symbols": top}, f)
    log.info(f"✅ Ranked and cached {len(top)} stocks")
    return top
