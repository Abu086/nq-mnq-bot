    ...
    while True:
        ...
        if (LIVE_TRADING_ENABLED and
            ...
            if success:
                ...
                log.warning(f"  ⚠️ Bull bot errored — will retry in {RETRY_COOLDOWN_MINUTES} min")
        ...
        if (LIVE_TRADING_ENABLED and
            ...
            success = run_script("live_nifty_bear_bot.py")
            if success:
                ran_today["india_bear"] = today
            else:
                log.warning(f"  ⚠️ Bear bot errored — will retry in {RETRY_COOLDOWN_MINUTES} min")
        ...
        # ── 9:20 AM IST — Strategy Four Breakout Bot ─────────────────────
        # Runs once at 9:20 AM — the bot manages its own scan loop until 3:15 PM
        if (STRATEGY_FOUR_ENABLED and
            ...
            log.info(f"\n{'='*50}")
            log.info(f"  9:20 AM IST — Starting Strategy Four Breakout Bot")
            log.info(f"{'='*50}")
            run_script("live_breakout_bot.py")
