...
# Track last retry attempt time (to space out retries, not hammer every 30s)
last_attempt = {
    "india_live": None,
}
...
def main():
    ...
    while True:
        ...
        # ── 9:20 AM IST onwards — Live NIFTY Bot (Mon/Wed/Thu/Fri — not expiry day) ──
        # Retries periodically through market hours (9:20 AM - 3:15 PM) if it
        # previously failed due to a technical error (rate limit, login, etc.)
        # Does NOT retry if it correctly completed or correctly skipped (bearish trend, holiday).
        if (now_ist.hour >= 9 and now_ist.hour < 15 and
                ...
                (last_attempt["india_live"] is None or
                 (now_ist - last_attempt["india_live"]).total_seconds() >= RETRY_COOLDOWN_MINUTES * 60)):

            last_attempt["india_live"] = now_ist
            log.info(f"\n{'='*50}")
            log.info(f"  Attempting LIVE NIFTY bot run")
            log.info(f"{'='*50}")
            success = run_script("live_nifty_bot.py")
            if success:
                # Script exited 0 → correctly skipped or correctly traded — done for today
                ran_today["india_live"] = today
            else:
                # Script exited non-zero → technical error — will retry after cooldown
                log.warning(f"  ⚠️ Live NIFTY bot reported an ERROR — will retry in {RETRY_COOLDOWN_MINUTES} min")
