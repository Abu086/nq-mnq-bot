...
def get_front_month_expiry(today: datetime.date) -> datetime.date:
    """
    Return the front-month NQ/MNQ options expiry (3rd Friday of nearest month).
    If today is past the 3rd Friday, roll to next month.
    """
    def third_friday(year: int, month: int) -> datetime.date:
        # Find first Friday of month, then add 2 weeks
    ...
    expiry = third_friday(today.year, today.month)

    # If today is on or past expiry, roll to next month
    if today >= expiry:
        if today.month == 12:
            expiry = third_friday(today.year + 1, 1)
        else:
            expiry = third_friday(today.year, today.month + 1)

    return expiry


def days_to_expiry(today: datetime.date) -> int:
    return (get_front_month_expiry(today) - today).days


# ── Simulate intraday P&L ─────────────────────────────────────────────────────
def simulate_trade(inst_name: str, inst: dict, spot: float) -> dict:
    """
    Simulate a Bull Call Spread entry at 10:00 ET and exit same day (intraday).
    Uses front-month expiry contract. No overnight risk.
    Premium scales with days-to-expiry (DTE) — more DTE = more premium.
    """
    today       = datetime.date.today()
    expiry      = get_front_month_expiry(today)
    ...
    # Premium scales with DTE: more time = more extrinsic value
    # Near expiry (DTE < 5): 20–30% of max value
    # Mid-month (DTE 5–15): 30–45%
    # Early month (DTE > 15): 40–55%
    if dte < 5:
        entry_pct = random.uniform(0.20, 0.30)
    elif dte < 15:
        entry_pct = random.uniform(0.30, 0.45)
    else:
    ...
    # Dynamically calculate contracts to fully utilise the $1,000 budget
    budget           = inst["budget"]
    fees_per_spread  = (inst["cme_fee"] + inst["broker_fee"] + inst["nfa_fee"]) * 2  # round trip
    cost_per_spread  = entry_premium + fees_per_spread
    n                = max(1, int(budget / cost_per_spread))  # at least 1 contract
    capital_used     = round(cost_per_spread * n, 2)

    # Max profit = max_value - entry_premium (per contract)
    max_profit_total = round((max_value - entry_premium) * n, 2)
    total_cost_paid  = round(entry_premium * n, 2)
    target_profit    = round(total_cost_paid * TARGET_PCT, 2)    # 30% of total cost paid
    stop_loss_amt    = round(total_cost_paid * SL_PCT, 2)        # 30% of total cost paid

    # Target exit: entry + 30% of entry premium per unit
    target_exit_premium = round(entry_premium * (1 + TARGET_PCT), 2)
    # Stop loss exit: entry - 30% of entry premium per unit
    sl_exit_premium     = round(entry_premium * (1 - SL_PCT), 2)

    # Simulate outcome: 55% hit target, 30% hit stop loss, 15% flat (end of day)
    rand = random.random()
    if rand < 0.55:
        outcome      = "PROFIT"
        # Exit exactly at target premium (30% of max profit captured)
        exit_premium = target_exit_premium
        exit_mins    = random.randint(30, 240)   # hit target within 30min–4hrs
    elif rand < 0.85:
        outcome      = "STOP LOSS"
        # Exit exactly at stop loss premium (lost 30% of premium paid)
        exit_premium = sl_exit_premium
        exit_mins    = random.randint(15, 180)   # stop loss hit within 15min–3hrs
    else:
        outcome      = "FLAT"
        # Exit near entry premium — neither target nor SL hit, closed at EOD
        exit_premium = round(entry_premium * random.uniform(0.95, 1.05), 2)
        exit_mins    = random.randint(300, 389)  # held till near close

    gross_pnl  = round((exit_premium - entry_premium) * n, 2)
    fees       = calc_fees(inst, n)
    net_pnl    = round(gross_pnl - fees["round_trip_total"], 2)
    return_pct = round(gross_pnl / total_cost_paid * 100, 1) if total_cost_paid else 0
    ...
    return {
        ...
        "total_cost":           capital_used,
        "max_value":            max_profit_total,
...
def _build_summary_sheet(ws):
    ...
    ws["B4"]  = f'=COUNTIF(NQ_Trades!Y3:Y1000,"PROFIT")'
    ws["B5"]  = f'=COUNTIF(NQ_Trades!Y3:Y1000,"STOP LOSS")'
    ws["B6"]  = f'=COUNTIF(NQ_Trades!Y3:Y1000,"FLAT")'
    ws["B7"]  = f'=IFERROR(B4/B3*100,0)'
    ws["B8"]  = f"=IFERROR(SUM(NQ_Trades!R3:R1000),0)"
    ws["B9"]  = f"=IFERROR(SUM(NQ_Trades!V3:V1000),0)"
    ws["B10"] = f"=IFERROR(SUM(NQ_Trades!W3:W1000),0)"
    ws["B11"] = 1000
    ws["B12"] = f"=IFERROR(B10/B11*100,0)"

    ws["B15"] = f"=COUNTA(MNQ_Trades!A3:A1000)"
    ws["B16"] = f'=COUNTIF(MNQ_Trades!Y3:Y1000,"PROFIT")'
    ws["B17"] = f'=COUNTIF(MNQ_Trades!Y3:Y1000,"STOP LOSS")'
    ws["B18"] = f'=COUNTIF(MNQ_Trades!Y3:Y1000,"FLAT")'
    ws["B19"] = f'=IFERROR(B16/B15*100,0)'
    ws["B20"] = f"=IFERROR(SUM(MNQ_Trades!R3:R1000),0)"
    ws["B21"] = f"=IFERROR(SUM(MNQ_Trades!V3:V1000),0)"
    ws["B22"] = f"=IFERROR(SUM(MNQ_Trades!W3:W1000),0)"
    ws["B23"] = 1000
...
def main():
    ...
    for inst_name, inst in INSTRUMENTS.items():
        ...
        log.info(f"  Expiry     : {trade['expiry']}  (DTE: {trade['dte']} days)")
