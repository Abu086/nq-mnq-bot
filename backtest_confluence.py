...
# ── Candidate Pool — NIFTY 100 constituents ───────────────────────────────
def fetch_nifty100_symbols() -> list:
    """Fetch the official, current NIFTY 100 constituent list from NSE Indices."""
    log.info(f"Fetching NIFTY 100 constituent list from {NIFTY100_CSV_URL}...")
    req = urllib.request.Request(NIFTY100_CSV_URL, headers={"User-Agent": "Mozilla/5.0"})
    with urllib.request.urlopen(req, timeout=20) as resp:
        raw = resp.read()
    df = pd.read_csv(io.BytesIO(raw))
    # Expected column name is 'Symbol' based on NSE Indices' standard format
    symbol_col = [c for c in df.columns if c.strip().lower() == "symbol"]
    if not symbol_col:
        raise RuntimeError(f"Could not find 'Symbol' column in NIFTY 100 CSV. "
                          f"Columns found: {df.columns.tolist()}")
    symbols = df[symbol_col[0]].dropna().astype(str).str.strip().tolist()
    log.info(f"  Found {len(symbols)} NIFTY 100 constituents")
    return symbols
