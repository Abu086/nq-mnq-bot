...
def run_live_trade() -> str:
    ...
    fees_est   = 40 + (0.0125/100 * net_debit * LOT_SIZE)
    cost_lot   = net_debit * LOT_SIZE + fees_est
    ...
    if not buy_order_id or not sell_order_id:
        log.error("❌ Entry order placement failed. Check Angel One dashboard.")
        if buy_order_id and not sell_order_id:
            log.error(f"⚠️  CRITICAL: Buy order {buy_order_id} placed but Sell leg FAILED — you may have an unhedged naked CALL position. Check Angel One dashboard immediately!")
        elif sell_order_id and not buy_order_id:
            log.error(f"⚠️  CRITICAL: Sell order {sell_order_id} placed but Buy leg FAILED — you may have an unhedged naked CALL position. Check Angel One dashboard immediately!")
        return "ERROR"

    entry_time = datetime.datetime.now(IST)
    log.info(f"  ✅ Entry complete at {entry_time.strftime('%H:%M IST')}")
