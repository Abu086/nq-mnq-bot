...
def fetch_option_ltp(client: AngelOneClient, symbol: str, retries: int = 3) -> float | None:
    ...
    for attempt in range(retries):
        try:
            ...
            if "rate" in str(e).lower() and attempt < retries - 1:
                ...
                log.warning(f"Rate limit hit for {symbol}, retrying in {wait}s...")
                time.sleep(wait)
                continue
...
def run_live_trade():
    ...
    buy_results  = client.search_scrip("NFO", buy_symbol)
    time.sleep(1)   # Angel One rate limit — avoid back-to-back calls
    sell_results = client.search_scrip("NFO", sell_symbol)
