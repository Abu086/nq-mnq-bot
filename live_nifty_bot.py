...
def run_live_trade() -> str:
    ...
    # ── STEP 0: Check REAL required margin for this basket BEFORE placing any order ──
    ...
    if margin_data is None:
        log.error("❌ Could not fetch margin requirement from Angel One. Aborting for safety — will not guess.")
        return "ERROR"

    required_margin = margin_data.get("totalMarginRequired")
    if required_margin is None:
        log.error(f"❌ Margin response missing 'totalMarginRequired' field: {margin_data}. Aborting for safety.")
        return "ERROR"

    required_margin = float(required_margin)
    log.info(f"  Required margin for this basket (with hedge benefit): Rs.{required_margin:,.2f}")
    log.info(f"  Available budget: Rs.{BUDGET:,.2f}")

    if required_margin > BUDGET:
        log.warning(f"  ⚠️  Required margin (Rs.{required_margin:,.2f}) exceeds budget (Rs.{BUDGET:,.2f}).")
        log.warning(f"  Skipping trade today — will NOT attempt order placement. No risk taken.")
        return "SKIPPED"

    log.info(f"  ✅ Margin check passed — proceeding with order placement.")
