"""
Strategy Eight -- Order Execution
------------------------------------------------------------------
Self-contained -- does not import from Strategy Four/Five/Six's files,
only the shared, generic angel_one_client.py. Identical mechanics to
Strategy Six's order module (proven in live trading): LIMIT order
priced 0.15% through LTP, poll-for-fill with terminal-status handling,
active cancel on timeout, and a real-position-at-broker fallback check
so a race between our cancel and an exchange fill is never mis-reported
as "not filled."
"""
...
def cancel_order(client, _request, order_id, variety="NORMAL"):
    """Actively cancel a still-resting order instead of leaving it to dangle
    unpredictably in the market."""
    try:
        body = {"variety": variety, "orderid": order_id}
        resp = _request("POST", CANCEL_ORDER_PATH, client._headers(auth=True), body)
